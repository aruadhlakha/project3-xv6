...
# Edit by Ceyhun
   ...
   def exec_and_test(self, args, libs = None, path = None,
         stdin = None, stdout = None, stderr = None, status = None):
      name = args[0]
      infd = None
      outfd = None
   ...
   def run_util(self, args):
      ...
      self.children.append(child)
      status = child.wait()
      if hasattr(self, 'status') and self.status is not None and status != self.status:
         raise Failure(str(self.name) + " returned incorrect status code. " +
               "Expected " + str(self.status) + " got " + str(status))
      return status
